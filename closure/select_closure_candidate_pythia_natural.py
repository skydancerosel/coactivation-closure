"""
select_closure_candidate_pythia_natural.py

Pick a closure-test candidate from Pythia 1B's natural-text Ising clusters.
Best ARI was at k=8 (ARI=0.35). Score each cluster by:
  - size (prefer 4-12 heads — interpretable range)
  - isolation ratio
  - class purity
"""
...
K_SPECTRAL = 8
RESULTS_DIR = Path("results/pythia_1b_nat_ising")
MECH_JSON = "/Volumes/Brandy/mini_gpt/.claude/worktrees/nostalgic-lederberg-80a58d/pythia_mechinterp_naturaltext.json"
N_LAYER = 16
N_HEAD = 8
MIN_SIZE = 4
MAX_SIZE = 12
...
for c in sorted(set(labels)):
    ...
    rec = {
        ...
        "layers": layers, "heads": head_list,
        "composition": composition, "n_classified": n_classified,
        "purity": float(purity), "isolation_ratio": iso_ratio,
        "within_mean_J": within_mean, "outside_mean_J": outside_mean,
    }
    clusters.append(rec)
...
if not candidates:
    candidates = clusters
    print(f"\n(no cluster in [{MIN_SIZE}, {MAX_SIZE}], using all)")
